...
import {
  getNotificationMode,
  getNotificationReminderHours,
  isAlwaysNotifyMode,
  isCronForceRunEnabled
...
function getComparableResult(result) {
  if (!result) {
    return null;
  }
...
function jobChanged(previous, current) {
  ...
  return JSON.stringify(getComparableResult(previous)) !== JSON.stringify(getComparableResult(current));
}

function hasReminderElapsed(job, currentCheckedAt) {
  const reminderHours = getNotificationReminderHours();
  if (!reminderHours) {
    return false;
  }

  const lastNotifiedAt = job.lastNotifiedAt ? new Date(job.lastNotifiedAt).getTime() : 0;
  if (!lastNotifiedAt) {
    return false;
  }

  const currentMs = currentCheckedAt ? new Date(currentCheckedAt).getTime() : Date.now();
  return currentMs - lastNotifiedAt >= reminderHours * 60 * 60 * 1000;
}

function shouldNotify(job, previous, current) {
  ...
  return hasReminderElapsed(job, current.checkedAt);
}

function getNotificationReason(job, previous, current) {
  if (isAlwaysNotifyMode()) {
    return "always_mode";
  }
  ...
  if (hasReminderElapsed(job, current.checkedAt)) {
...
export async function runJobNow(jobId) {
  ...
  const notify = shouldNotify(job, previousResult, currentResult);
  const notificationReason = getNotificationReason(job, previousResult, currentResult);
  ...
  await updateJob(jobId, (entry) => ({
    ...
    lastChangedAt: jobChanged(previousResult, currentResult)
      ...
      : entry.lastChangedAt,
    lastNotifiedAt: notify ? currentResult.checkedAt : entry.lastNotifiedAt,
